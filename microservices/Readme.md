...
4. [Микросервисы: масштабирование](4.Микросервисы__масштабирование.pdf).

#### Полезные источники:

[microservices.io by Chris Richardson](https://microservices.io/) – ресурс, содержащий полную информацию о проектировании и реализации микросервисов. Сайт обязательно должен быть добавлен в закладки вашего браузера, если вы занимаетесь проектированием микросервисной архитектуры.

[Series of articles on the NGINX blog begining with introduction to microservices](https://www.f5.com/company/blog/nginx/introduction-to-microservices) – серия статей от компании NGINX, демонстрирующая принципы применения nginx в микросервисной архитектуре.

[Microservices – Martin Fowler](https://martinfowler.com/articles/microservices.html) - заметки о микросервисах от известного архитектора программного обеспечения. Статья позволит понять, что из себя представляют микросервисы с точки зрения индустрии, какие плюсы они привносят в систему и какие недостатки скрывают.
...
### Задача 1: Кластеризация решения для обеспечения развёртывания, запуска и управления приложениями

В качестве решения для развёртывания, запуска и управления микросервисной архитектурой предлагается использовать комбинацию российских продуктов: платформу оркестрации контейнеров **«Штурвал»** (разработчик — «Лаборатория Числитель») и Service Mesh **Platform V Synapse** от «СберТеха».

#### Соответствие требованиям

| № | Требование | Реализация в решении |
| :--- | :--- | :--- |
| **1** | Поддержка контейнеров | «Штурвал» — полноценная платформа оркестрации контейнеров на базе Kubernetes, управляет контейнерами (Docker, containerd). |
| **2** | Обнаружение сервисов и маршрутизация запросов | **Platform V Synapse** (Service Mesh) обеспечивает обнаружение сервисов, балансировку нагрузки и гибкую маршрутизацию на уровне sidecar-прокси (аналог Istio). |
| **3** | Горизонтальное масштабирование | «Штурвал» поддерживает ReplicaSet и Deployment для увеличения количества реплик сервисов (ось X масштабирования). |
| **4** | Автоматическое масштабирование | Встроенный Horizontal Pod Autoscaler (HPA) автоматически масштабирует поды на основе метрик CPU, RAM и кастомных метрик. |
| **5** | Явное разделение ресурсов (внешние / внутренние) | Ingress Controller (или Gateway) управляет внешним доступом, внутреннее общение — через ClusterIP Services. Service Mesh добавляет политики доступа. |
| **6** | Конфигурирование переменными среды и безопасное хранение секретов | **ConfigMap** для обычных настроек, **Secrets** для паролей, ключей, токенов. Secrets могут быть зашифрованы и интегрированы с HashiCorp Vault. |
---

#### Обоснование выбора на основе лекции

**1. Масштабирование**
- **Ось X** — горизонтальное масштабирование через HPA в «Штурвале».
- **Ось Y** — разделение на микросервисы (разные Deployment).
- **Ось Z** — шардирование данных (StatefulSet + PersistentVolume).

**2. Повышение надёжности**
Platform V Synapse реализует **Retry**, **Circuit Breaker**, **Timeout**, **Rate Limiting** на уровне sidecar-прокси без изменения кода сервисов.

**3. Балансировка нагрузки и маршрутизация**
Platform V Synapse Gateway обеспечивает терминацию SSL, проверку работоспособности, контентно-зависимую маршрутизацию и канареечные развёртывания.

**4. Автомасштабирование**
HPA масштабирует поды на основе метрик CPU, памяти или кастомных метрик (например, количество запросов в секунду).

**5. Service Discovery**
Platform V Synapse предоставляет DNS-имена для сервисов и регистрацию через Service Registry.

**6. Service Mesh**
- Выносит в инфраструктуру управление трафиком, observability и безопасность.
- Обеспечивает mTLS-шифрование между сервисами (Citadel).
- Упрощает реализацию сложных стратегий развёртывания (Blue/Green, Canary, A/B testing).

**7. Хранение секретов и конфигураций**
- **ConfigMap** — для обычных настроек.
- **Secrets** — для чувствительных данных. Могут быть зашифрованы и интегрированы с HashiCorp Vault.

#### Дополнительные преимущества

| Преимущество | Как обеспечивается |
| :--- | :--- |
| **Импортозамещение** | Оба продукта входят в реестр отечественного ПО. |
| **Промышленная надёжность** | Решения проверены в крупнейших российских компаниях (Сбер, Газпром, РЖД). |
| **Официальная совместимость** | Совместимость «Штурвала» и Platform V Synapse подтверждена разработчиками. |
| **Технологическая независимость** | Полная замена зарубежных решений (Kubernetes + Istio). |
| **Безопасность** | mTLS, шифрование секретов, сертификация ФСТЭК (по требованию). |
---
