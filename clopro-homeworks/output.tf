...
# Публичный IP внешнего NLB (из первого listener'а)
output "nlb_public_ip" {
  value = tolist(tolist(yandex_lb_network_load_balancer.nlb.listener)[0].external_address_spec)[0].address
}

output "nlb_url" {
  value = "http://${tolist(tolist(yandex_lb_network_load_balancer.nlb.listener)[0].external_address_spec)[0].address}/"
}
